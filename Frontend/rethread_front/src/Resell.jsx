...
function Resell() {
  ...
  const [filteredResell, setFilteredResell] = useState([]);
  ...
  useEffect(() => {
    axios.get(`http://localhost:10000/${categoryMap[category]}/${filteredWords}/Resell`)
      .then((response) => {
        setResell(response.data);
        setFilteredResell(response.data)
      })
      .catch((error) => {
        console.log(error);
      });
  ...
  { console.log("This is it", filteredResell) }
  ...
  const handleCheckClick = (newSelectedColors, newSelectedSizes) => {
    const newFilteredResell = Resell.filter(product =>
    ...
    // Update state with filtered Resell
    setFilteredResell(newFilteredResell);
  }
  ...
  const handleSearchClick = () => {
    ...
    const filteredResell = Resell.filter(product =>
      product.product_name.toLowerCase().includes(searchQuery.toLowerCase())
    );
    // Update state with filtered Resell
    setFilteredResell(filteredResell);
    { console.log(filteredResell) }
  }
    ...
    <>
      <div style={{ display: 'flex', minHeight: '100vh' }}>
        ...
        <div style={{ flex: 1, padding: '20px' }}>
          <h1>Showing Results for Resell</h1>
          ...
          >
            {Array.isArray(filteredResell) && filteredResell.map((product) => (
              <Card
